  ...
  <div class="app-container">
      ...
      <el-col :span="20">
        <div class="grid-content bg-purple-light">
          ...
          <el-table :data="list" 
            ...
            <el-table-column align="center" label='模块名称' width="150">
              <template slot-scope="scope">
                {{scope.row.name}}
              </template>
            </el-table-column>

            <el-table-column label="请求地址" width="150" align="center">
              <template slot-scope="scope">
                {{scope.row.url}}
              </template>
            </el-table-column>
            ...
            <el-table-column label="矢量图标" width="150" align="center">
              <template slot-scope="scope">
                {{scope.row.vector}}
              </template>
            </el-table-column>

            <el-table-column label="叶子节点" width="150" align="center">
              <template slot-scope="scope">
                {{scope.row.is_leaf}}
              </template>
            </el-table-column>
    ...
    <el-dialog title="新增参数" :visible.sync="isDialogShow">
      <el-form :rules="rules" 
        ...
        <el-form-item label="矢量图标" prop="vector">
          <el-input v-model="form.vector"></el-input>
        </el-form-item>
...
import { getTreeData, getList, saveModule, updateModule, deleteModule} from '@/api/systeManage/resource/functionalModule'
...
export default {
  ...
  methods: {
    ...
    async confirmEditBtn (){
      try {
        let response = await updateModule(this.form)
        this.fetchData(this.listQuery)
      } catch (err){
        console.log(err)
      }
